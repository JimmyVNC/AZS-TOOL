...
// be restored by macOS after the callback and appear not to reverse.
typedef struct __IOHIDEvent *IOHIDEventRef;
typedef double IOHIDFloat;
static const uint32_t kAZSIOHIDScrollX = ((uint32_t)6 << 16) | 0;
static const uint32_t kAZSIOHIDScrollY = ((uint32_t)6 << 16) | 1;
extern "C" IOHIDEventRef CGEventCopyIOHIDEvent(CGEventRef);
extern "C" IOHIDFloat IOHIDEventGetFloatValue(IOHIDEventRef, uint32_t);
extern "C" void IOHIDEventSetFloatValue(IOHIDEventRef, uint32_t, IOHIDFloat);

static void AZSReverseScrollHIDPayload(CGEventRef event) {
    IOHIDEventRef hid = CGEventCopyIOHIDEvent(event);
    if (hid == NULL) return;
    IOHIDEventSetFloatValue(hid, kAZSIOHIDScrollY,
                            -IOHIDEventGetFloatValue(hid, kAZSIOHIDScrollY));
    IOHIDEventSetFloatValue(hid, kAZSIOHIDScrollX,
                            -IOHIDEventGetFloatValue(hid, kAZSIOHIDScrollX));
    CFRelease(hid);
}
...
extern "C" {
    ...
    CGEventRef MKEngineCallback(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *refcon) {
        ...
        // callback is synchronous, so returning NULL really suppresses a mapped
        // mouse button or an external-display media key.
        // Ordinary key events must continue into the Vietnamese engine when
