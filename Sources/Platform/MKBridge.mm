...
+ (BOOL)startEventTap {
    ...
    // Keep every global utility on this known-good Accessibility event tap.
    // Creating a second active tap can silently fail on recent macOS versions,
    // which previously made volume keys, extra mouse buttons, and scrolling all
    // appear enabled in the UI while receiving no events.
